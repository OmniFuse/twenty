  ...
  async sendInvitations(
    ...
    for (const invitation of invitationsPr) {
      if (invitation.status === 'fulfilled') {
        ...
        const joinTeamMsg = msg`Присоединяйтесь к вашей команде на Liddex`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via Liddex) <${this.twentyConfigService.get('EMAIL_FROM_ADDRESS')}>`,
